...
client.on("message", async message => {
  ...
  console.log(message.channel.id)
    if (message.channel.id != "785315159895048212") {
        return};
    if (command == "wl") {
        console.log(args)
        if (args[0] !== undefined) {
...
function activity() {
    setTimeout(() => {
      fivereborn.query("149.202.65.94", 30120, (err, data) => {
        if (err) {
          console.log(err);
        } else {
          client.user.setActivity(" En ligne " + data.clients + "/" + data.maxclients, { type: "STREAMING",url: "https://www.twitch.tv/parowww" });
        }
      });
      activity();
    }, 10000);

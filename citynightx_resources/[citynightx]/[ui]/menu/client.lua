...
local props = {
    ...
    ["doubleshootgun"] = "w_sr_sniperrifle",
...
Citizen.CreateThread(
    function()
        SetNuiFocus(false, false)
        Crosshair(false)
        Items = exports["Citynight"]:GetItemsData()
        while true do
        ...
        while true do
            ...
            if carrying then
                ...
                if IsDisabledControlJustPressed(0, 47) then
                    ...
                    exports["Citynight"]:TriggerPlayerEvent("newProps", -1, carryingItem, coords)
                    carrying = false
        ...
        if not GetPlayerServerIdInDirection(3.0) then
            ...
            if
                DoesEntityExist(obj[current[1]].obj) and
                    exports["Citynight"]:canReceive(current[2].item[1].name, #current[2].item)
             then
                randPickupAnim()
                for i = 1, #current[2].item, 1 do
                    exports["Citynight"]:AddItemtoInv(current[2].item[i])
                end
                exports["Citynight"]:TriggerPlayerEvent("deletePickup", -1, current[1])
            elseif not exports["Citynight"]:canReceive(current[2].item[1].name, #current[2].item) then
            -- exports["Citynight"]:ShowNotification("~r~Vous n'avez pas assez de place !")
            end
        else
            exports["Citynight"]:ShowNotification("~r~Un joueur est trop proche de vous (anti-glitch)")
        end
    end
)
local max = 4.5

...
RMenu.Add('ponsonbys',"main", RageUI.CreateMenu("Ponsonbys", "Actions disponibles",10,100))
local pedPrev
...
local function spawnManequin()
    ...
    pedPrev = CreatePed(3, skinLIST[skinIndex],-706.05,-151.02,36.41, 258.55, true,true)
...
local function respawnSkin()
    ...
    pedPrev = CreatePed(4, skinLIST[skinIndex],-706.05,-151.02,36.41, 258.55, true,true)
...
RegisterNetEvent("ENTI:DeletePed")
AddEventHandler("ENTI:DeletePed",function()
    ClearAreaOfPeds(-706.05,-151.02,36.41, 258.55,3.0,1)
end)
RMenu:Get("ponsonbys","main").Closed = function()
            ...
            RageUI.DrawContent({ header = true, glare = false }, function()
                RageUI.Button(pedPrev == nil and "Faire apparaitre un mannequin" or "Supprimer le mannequin",nil,{},true,function(_,_,Selected)
                    ...
                    elseif Selected and pedPrev ~= nil then
                        NetworkRequestControlOfEntity(pedPrev)
                        DeleteEntity(pedPrev)
                        TriggerPlayerEvent("ENTI:DeletePed",-1)
                        pedPrev = nil
                    end
...
elseif Selected then
                    ...
                    if not lockedTorso and not lockedPant and not lockedPantColor and not lockedchauss and not lockedPant and not lockedchaussColor and not accessLocked and not accessColorLocked and not underscolorLocked and not topLocked and not topColorLocked  then
                        ...
                        RageUI.Button("Valider" , nil, {
        
                             }, true, function(Hovered, Active, Selected)
                                if Selected then
                                    
                                    local data = {torso=torsoPos-1,pant=pantPos-1,chaus=chaussPos-1,unders=undersPos-1,access=accessPos-1,tops=topsPos-1,pantcolor=pantcolorPos-1,chausscolor=chausscolorPos-1,underscolor=underscolorPos -1,topcolor=topcolorPos-1,accesscolor=accesscolorPos-1}
                                    local item = {}
                                    item.name = "tenue"
                                    item.data = data
                                    ...
                                    if count ~= nil then
                                        NetworkRequestControlOfEntity(pedPrev)
                                        SetEntityCoords(pedPrev, 0,0, -120.0)
                                        DeleteEntity(pedPrev)
                                        TriggerPlayerEvent("ENTI:DeletePed",-1)
                                        pedPrev = nil
                                        RageUI.Refresh()

...
RegisterServerEvent("orga:UpdateJob2")
AddEventHandler("orga:UpdateJob2", function(job,rank,src)
    local ply = Player.GetPlayer(src)
    local uuid = ply.uuid
    MySQL.Async.execute(
...
RegisterServerCallback('jobs:GetEmploy2', function(source, callback,job)
    MySQL.Async.fetchAll('SELECT * FROM players_jobs WHERE orga = @job', {
        ["@job"] = job
    }, function(result)
        MySQL.Async.fetchAll('SELECT * FROM players_identity', {
            ["@job"] = job
...
AddEventHandler("criminalrecords:Add", function(title,desc,coupable,author)
    MySQL.Async.execute(
        'INSERT INTO criminal_records (`title`,`desc`,`author`,`oupable) VALUES(@title,@desc,@author,@coupable)',
        {
            ['@title'] = title,
            ['@desc'] = desc,
            ['@coupable'] = coupable,

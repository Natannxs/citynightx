...
local Items = {
    ...
    {item="hammer",data=json.encode({serial=math.random(111111111,999999999)}),chance=2},
    {item="bottle",data=json.encode({serial=math.random(111111111,999999999)}),chance=2},
}
...
local distributor = {
    ...
    ["prop_vend_snak_01"] = { it = "chips", max = 1 },
}
...
local function BuyCan()
    ...
    if found then
        if Inventory.CanReceive(v.it,1) then
            dataonWait= {detail="Achat distributeur", price=5,fct=function()

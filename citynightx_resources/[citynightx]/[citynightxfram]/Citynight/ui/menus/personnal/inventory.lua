                ...
                RageUI.DrawContent( 
                    ...
                    function() 
                        RageUI.Button( 
                            ...
                            function(_, _, Selected) 
                                if Selected then 
                                    if Inventory.SelectedItem.name == "tel" and Inventory.SelectedItem.data.code ~= nil then   
                                        TriggerEvent("askforNumber") 
                                        local n = nil 
                                        Citizen.CreateThread(function() 
                                            RegisterNetEvent("codeis") 
                                            AddEventHandler("codeis",function(_h) 
                                                if _h == nil then 
                                                    n = "not"
                                                else
                                                    n = _h 
                                        ...
                                        while n == nil do Wait(1) end 
                                        print(n,Inventory.SelectedItem.data.code)
                                        if tostring(n) == tostring(Inventory.SelectedItem.data.code) then   
                                            RageUI.GoBack() 
                                            Wait(10) 
                                            Inventory:UseItem() 
                                        else 
                                            ShowNotification("~r~Mauvais code !") 
                                        end 
                                    else 
                                        RageUI.GoBack() 
                                        Wait(10) 
                        ...
                        if Inventory.SelectedItem.name == "tel" then 
                            if Inventory.SelectedItem.data.code == nil then 
                                RageUI.Button("Définir un code",nil,{},true,function(_, _, Selected) 
                                    if Selected then 
                                        -- local num = tostring(KeyboardInput("Code (4 chiffres)",nil,4)) 
                                        -- if num ~= nil and #num == 4 then 
                                        --     Inventory.SelectedItem.data.code = num 
                                        --     ShowNotification("~g~Code définis") 
                                        -- end 
                                        ...
                                        if n ~= "not" then
                                            Inventory.SelectedItem.data.code = n 
                                            ShowNotification("~g~Code définis") 
                                        end
                                    end 
                                end) 
                            else 
                                RageUI.Button("Changer le code",nil,{},true,function(_, _, Selected) 
                                    if Selected then 
                                        local old = tostring(KeyboardInput("Code actuel",nil,4)) 
                                        print(old,Inventory.SelectedItem.data.code)
                                        if old == tostring(Inventory.SelectedItem.data.code) then 
                                            TriggerEvent("askforNumber") 
                                            local n = nil 
                                            Citizen.CreateThread(function() 
                                                RegisterNetEvent("codeis") 
                                            ...
                                            if n ~= "not" then
                                                Inventory.SelectedItem.data.code = n 
                                                ShowNotification("~g~Code changé") 
                                            end
                                        else 
                                            ShowNotification("~r~Code incorrect") 
                                        end 
    ...
    function() 
        while true do 
            ...
            if Job:Get() ~= nil then 
                ...
                if #json.encode(Inventory.Inventory) ~= oldTableLenght and #json.encode(Inventory.Inventory) ~= #json.encode({}) then 

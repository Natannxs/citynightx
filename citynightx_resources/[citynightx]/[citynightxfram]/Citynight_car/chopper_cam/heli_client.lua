...
local aw139_hash = GetHashKey("aw139")
local buzzart_hash = GetHashKey("buzzard2")
local seasparrow_hash = GetHashKey("seasparrow")
    ...
    function()
        while true do
                ...
                if IsHeliHighEnough(heli) then
                    ...
                    if IsControlJustPressed(0, toggle_rappel) then -- Initiate rappel
                        ...
                        if GetPedInVehicleSeat(heli, 1) == lPed or GetPedInVehicleSeat(heli, 2) == lPed then
                            ...
                            AddTextComponentString("~r~Le pilote ne peut pas faire du rappel")
            ...
            if helicam then
                ...
                while helicam and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == heli) and
                    IsHeliHighEnough(heli) do
                    if IsControlJustPressed(0, toggle_helicam) then -- Toggle Helicam
                        PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
                        helicam = false
                    end
...
function IsPlayerInPolmav()
    ...
    return IsVehicleModel(vehicle, polmav_hash) or IsVehicleModel(vehicle, frogger_hash) or
        ...
        IsVehicleModel(vehicle, seasparrow_hash)
end
...
function GetVehicleInView(cam)
    ...
    local rayhandle =
        CastRayPointToPoint(coords, coords + (forward_vector * 200.0), 10, GetVehiclePedIsIn(PlayerPedId()), 0)

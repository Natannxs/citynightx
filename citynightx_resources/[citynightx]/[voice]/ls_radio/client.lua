...
Radio.Labels = {        
    { "FRZL_RADIO_HELP", "~s~" .. (radioConfig.Controls.Secondary.Enabled and "~" .. radioConfig.Controls.Secondary.Name .. "~ + ~" .. radioConfig.Controls.Activator.Name .. "~" or "~" .. radioConfig.Controls.Activator.Name .. "~") .. " pour sortir/ranger la radio.~n~~" .. radioConfig.Controls.Toggle.Name .. "~ pour allumer la radio ~g~on~s~.~n~~" .. radioConfig.Controls.Decrease.Name .. "~ ou ~" .. radioConfig.Controls.Increase.Name .. "~ pour changer la fréquence~n~~" .. radioConfig.Controls.Input.Name .. "~ pour chosir la fréquence~n~~" .. radioConfig.Controls.ToggleClicks.Name .. "~ pour ~a~ les clics radio~n~Fréquence: ~1~ MHz" },
    { "FRZL_RADIO_HELP2", "~s~" .. (radioConfig.Controls.Secondary.Enabled and "~" .. radioConfig.Controls.Secondary.Name .. "~ + ~" .. radioConfig.Controls.Activator.Name .. "~" or "~" .. radioConfig.Controls.Activator.Name .. "~") .. " pour sortir/ranger la radio.~n~~" .. radioConfig.Controls.Toggle.Name .. "~ pour éteindre la radio ~r~off~s~.~n~~" .. radioConfig.Controls.Broadcast.Name .. "~ pour parler en radio.~n~Fréquence: ~1~ MHz" },
...
Radio.Commands = {
    {
        ...
        Handler = function(src, args, raw)
            ...
            elseif (Radio.Open or Radio.On) and ((not Radio.Enabled) or (not Radio.Has) or isDead) then
                ...
                Radio.On = false
                Radio:Remove()
                radioConfig.radioEnabled = false
...
function Radio:Toggle(toggle)
    ...
    if self.On and not radioConfig.AllowRadioWhenClosed then
        radioConfig.radioEnabled = toggle
...
function Radio:Add(id)
    ...
    exports.tokovoip_script:setPlayerData(playerName, "radio:channel", id, true);
    exports.tokovoip_script:addPlayerToRadio(id)
    --ESX.ShowNotification(Config.messages['joined_to_radio'] .. id .. '.00 MHz </b>')
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        --[[if isActivatorPressed and isSecondaryPressed and not isFalling and Radio.Enabled and Radio.Has and not isDead then
            Radio:Toggle(not Radio.Open)
        else]]if (Radio.Open or Radio.On) and ((not Radio.Enabled) or (not Radio.Has) or isDead) then
            Radio:Remove()
            radioConfig.radioEnabled = false
            --exports["mumble-voip"]:SetMumbleProperty("radioEnabled", false)
            Radio:Toggle(false)
        ...
        if Radio.Open then
            ...
            if IsControlJustPressed(0, radioConfig.Controls.Toggle.Key) then
                ...
                radioConfig.radioEnabled = Radio.On
            ...
            if not Radio.On then
                ...
                if IsDisabledControlJustPressed(0, radioConfig.Controls.ToggleClicks.Key) then
                    ...
                    radioConfig.micClicks = Radio.Clicks
...
Citizen.CreateThread(function()
	while true do
        ...
        if NetworkIsSessionStarted() then
            radioConfig.radioClickMaxChannel = radioConfig.Frequency.Max
            radioConfig.radioEnabled = false
...
RegisterNetEvent("Radio.OffON")
AddEventHandler("Radio.OffON", function()
    if Radio.Has then
        Radio.On = not Radio.On
...
RegisterNetEvent("Radio.AddDel")
AddEventHandler("Radio.AddDel", function(idJoueur, frequ, value)
    if value == "add" then
        TriggerServerEvent("TokoVoip:addPlayerToRadio", frequ, idJoueur)
        exports.tokovoip_script:setPlayerData(idJoueur, "radio:channel", frequ, true)
        TriggerEvent("Radio.OffON", false)
    elseif value == "del" then
        TriggerServerEvent("TokoVoip:removePlayerFromRadio", frequ, idJoueur)
        exports.tokovoip_script:setPlayerData(idJoueur, "radio:channel", "nil", true)
        TriggerEvent("Radio.OffON", true)
    end
end)

RegisterCommand( "radio_open", function()
    if Radio.Has then
        TriggerEvent('Radio.Set', true)
        TriggerEvent('Radio.Toggle')
    else
        exports["Citynight"]:ShowNotification("~r~Vous n'avez pas de radio équipée")
    end
end, false)
RegisterKeyMapping( "radio_open", "Ouvrir/Fermer la radio", "keyboard", "l")